* About
This is a wrapper around the official [[https://github.com/FYNCH-BIO/dpu/tree/master]['dpu']] code that runs the eVOLVER platform. Use the contents of this repository instead of the template provided in the official repo.

NOTE: This readme contains internal instructions to setup and run a typical eVOLVER experiment. This can be directly applicable to your setup if you run your eVOLVER scripts out of =~/Documents/dpu/experiment/=. If not, please modify the file paths accordingly.

** Requirements
   Apart from the instructions for a standard eVOLVER setup (see  https://khalil-lab.gitbook.io/evolver/getting-started/software-installation/dpu-installation), this wrapper requires the =streamlit= package which drives the front end for easy, custom experiment definition. 
** Setup 
Copy this repo

* Experiment setup instructions (INTERNAL USE ONLY, USE WITH CAUTION)
...
6. Start the visual interface to populate the configuration file.
   #+begin_src 
bash monitor.sh
   #+end_src
   This will return open a browser tab with the streamlit interface to
...
9. If you are running a calibration:
   ...
   7. Create the calibration file
      #+begin_src bash
python write_calibration_to_file.py
      #+end_src
      This will create  a CSV file with the name of the calibration experiment, the same
